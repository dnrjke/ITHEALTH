...
function initI18n() {
    const translations = {
        KR: {
            tabs: { headache: '두통/무기력', turtle: '거북목', hand: '손의 통증' },
            navDesc: { headache: '깊은 호흡과 자세 개선', turtle: '목과 어깨 건강', hand: '손목과 손가락 케어' },
            hero: {
                title: '건강한 IT 라이프',
        ...
        EN: {
            tabs: { headache: 'Headache/Fatigue', turtle: 'Text Neck', hand: 'Hand Pain' },
            navDesc: { headache: 'Deep breathing & posture', turtle: 'Neck & shoulder health', hand: 'Wrist & finger care' },
            hero: {
        ...
        JP: {
            tabs: { headache: '頭痛・無気力', turtle: 'スマホ首', hand: '手の痛み' },
            navDesc: { headache: '深い呼吸と姿勢改善', turtle: '首と肩の健康', hand: '手首と指のケア' },
            hero: {
                title: '健康的なITライフ',
...
function initChecklist() {
        ...
        function toggleBreakGuide() {
            const breakGuideSection = document.getElementById('break-guide-section');
            const quickBreakBtn = document.querySelector('.quick-break-btn');
            
            if (breakGuideSection) {
                const isExpanded = breakGuideSection.classList.contains('expanded');
                
                if (isExpanded) {
                    breakGuideSection.classList.remove('expanded');
                    quickBreakBtn.textContent = '5분 휴식 가이드';
                    
                    setTimeout(() => {
                        quickBreakBtn.scrollIntoView({ 
                            behavior: 'smooth', 
                            block: 'center' 
                        });
                    }, 100);
                } else {
                    breakGuideSection.classList.add('expanded');
                    quickBreakBtn.textContent = '가이드 접기';
                    
                    setTimeout(() => {
                        breakGuideSection.scrollIntoView({ 
                            behavior: 'smooth', 
                            block: 'start' 
                        });
                    }, 200);
    ...
    const quickBreakBtn = document.querySelector('.quick-break-btn');
        ...
        checkInputs.forEach(input => {
            input.addEventListener('change', updateCheckCount);
        });
...
function initTimer() {
    ...
    const globalTimerStart = document.getElementById('global-timer-start');
    if (!globalTimerStart) {
        console.warn('⚠️ 타이머 요소를 찾을 수 없습니다. 타이머 초기화를 건너뜁니다.');
        return;
    }
    ...
    function jumpToStep(targetStepNum) {
        ...
        // 해당 단계의 시작 시간 계산
        let stepStartTime = 0;
        for (const stepData of timerState.steps) {
            if (stepData.step === targetStepNum) {
                break;
            }
            stepStartTime += stepData.duration;
        }
    ...
    function checkGlobalStepTransition() {
        let elapsedTime = 0;
        for (const stepData of timerState.steps) {
            elapsedTime += stepData.duration;
            if (timerState.currentTime <= elapsedTime) {
                if (timerState.currentStep !== stepData.step) {
                    timerState.currentStep = stepData.step;
                    updateCardStates(stepData.step);
                }
                break;
            }
        }
    ...
    // 전체 타이머 버튼
    document.getElementById('global-timer-start').addEventListener('click', () => {
