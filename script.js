...
function initNavigation() {
    ...
    function scrollToTab(tabIndex) {
        ...
        let alignment = 'center'; // 기본은 중앙 정렬

        // 💡 양 끝 항목 예외 처리 (화면 밀림 방지)
        
        // 1. 가장 첫 번째 항목(Index 0) 처리: 'start' 정렬로 강제
        if (tabIndex === 0) {
            alignment = 'start';
        } 
        // 2. 가장 마지막 항목 처리: 'end' 정렬로 강제
        ...
        if (targetItem && navCarouselContainer && window.innerWidth <= 768) {
            targetItem.scrollIntoView({ 
                behavior: 'smooth', 
                inline: alignment, // 동적으로 'start', 'end', 'center' 적용
                block: 'nearest' 
            });
        }
        
        currentTabIndex = tabIndex;
        // 💡 오직 항목 선택 시에만 도트 상태를 업데이트합니다.
...
function initTimer() {
    ...
    function setupProgressBarInteraction() {
        ...
        progressContainers.forEach(container => {
            ...
            function seekToPosition(clientX) {
                ...
                // 시간 업데이트
                timerState.currentTime = newTime;
    ...
    function startGlobalTimer(guideType = 'rest-all') {
        ...
        if (stickyProgressBar) {
            ...
            stickyProgressBar.classList.remove('completed');
        }
        
        // 시간 표시 초기화
        const stickyCurrentTime = document.getElementById('sticky-current-time');
    ...
    function startIndividualTimer(stepNum) {
        ...
        if (stepTimer) {
            ...
            const currentTime = stepTimer.querySelector('.step-current-time');
        ...
        if (stepTimer) stepTimer.classList.remove('hidden');
    ...
    function resetTimer() {
        ...
        const currentStepNum = timerState.currentStep;
        ...
        if (wasGlobalMode) {
        ...
        } else if (timerState.mode === 'individual' && currentStepNum) {
            const stepTimer = document.querySelector(`[data-step-timer="${currentStepNum}"]`);
            if (stepTimer) {
            ...
            const playBtn = document.querySelector(`.step-play-btn[data-step="${currentStepNum}"]`);
            if (playBtn) {
                playBtn.classList.remove('playing');
                playBtn.querySelector('span').textContent = '▶️';
            }
        ...
        timerState.scrollPosition = 0;
        timerState.steps = [];
        
        // 이전 단계 추적 변수 초기화
        previousStepNum = null;
        
        console.log('✅ 타이머 리셋 및 UI 초기화 완료');
    }
    ...
    function completeTimer() {
        ...
        if (wasGlobalMode) {
            ...
            const stepTimer = document.querySelector(`[data-step-timer="${currentStepNum}"]`);
            if (stepTimer) stepTimer.classList.add('hidden');
        ...
        setTimeout(() => {
            ...
            document.querySelectorAll('[data-step-timer]').forEach(timer => {
                ...
                const currentTime = timer.querySelector('.step-current-time');
                const totalTime = timer.querySelector('.step-total-time');
                if (currentTime) currentTime.textContent = '0:00';
                // totalTime은 그대로 유지
            });
            ...
            console.log('✅ 타이머 완료 및 UI 초기화 완료');
        }, 3000);
    }
